...
def compute_disc_prob(A, nb, C, minN, W_base, latency):
    ...
    Pnc = (1-P_c) ** gamma
    res = L_Scan - latency % (A+max_slop/2)
    p_double = res/W_base
    Pnc = (1-p_double)*Pnc + p_double*(2*Pnc-Pnc**2)
    Pd_one = P_in * Pnc
...
for n_chunk in n_chunk_list:
    for sla_prob in sla_prob_list:
        for latency in latency_list:
            ...
            out_file_name = f"./[Extended+AE] Final Result/BLEndExtAE Results/ExtendedAE_blend_{sla_prob}_{latency/1000}s_{n_chunk}chunk_log.txt"
            ...
            with open(out_file_name, 'w') as con:
                ...
                for sla_nodes in sla_node_set:
                    ...
                    minN = n_chunk # mininum number of windows that needs to be captured 
                    for W_base in range(int(latency/(minN+1))//2, int(4*L_Scan), -1):
                        E = W_base

                        advTime = E - L_Scan
                        psiGap = advTime % (A + max_slop / 2) 
